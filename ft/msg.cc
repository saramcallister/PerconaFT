...
ft_msg ft_msg::deserialize_from_rbuf(struct rbuf *rb, XIDS *x, bool *is_fresh) {
	...
	t.ref_count = 1; //single point msgs
    }
    t.type = raw_type;
...
int & ft_msg::ref_count_of_broadcast_msg() {
    assert(ft_msg_type_applies_all(_type.type));
...
ft_msg_type ft_msg::type_all() const {
    return _type;
}
...
size_t ft_msg::total_size() const {
    ...
    static size_t msg_overhead;
    if (ft_msg_type_applies_all(_type.type)) {
...
void ft_msg::serialize_to_wbuf(struct wbuf *wb, bool is_fresh) const {
    wbuf_nocrc_char(wb, (unsigned char) _type.type);
    if (ft_msg_type_applies_all(_type.type))
