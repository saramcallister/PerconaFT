...
inline const char * construct_dbg_context_for_write_node(const char * fname, bool for_checkpoint,  const char * func_name) {
    char * dbg_context = (char *) toku_malloc(sizeof("Called by %s: writing to file %s for checkpoint")+ sizeof(func_name)+ PATH_MAX); 

    if(for_checkpoint)
        sprintf(dbg_context, "Called by %s: writing to file %s for checkpoint", func_name, fname);
    else
        sprintf(dbg_context, "Called by %s; writing to file %s for eviction", func_name, fname);

    return (const char *) dbg_context;

}

inline const char * construct_dbg_context_for_write_others(const char * fname, const char * func_name, const char * others) {
    char * dbg_context = (char *) toku_malloc(sizeof("Called by %s: writing to file %s for %s")+ sizeof(func_name)+ PATH_MAX + sizeof(others));
    sprintf(dbg_context, "Called by %s: writing to file %s for %s", func_name, fname, others);
    return (const char *) dbg_context;
