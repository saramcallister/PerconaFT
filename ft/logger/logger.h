...
inline const char * construct_dbg_context_for_logging(char * fname, const char * func_name, const char * others) {
    char * dbg_context = (char *) toku_malloc(sizeof("Called by %s: writing to file %s for %s")+ sizeof(__func__)+ PATH_MAX + sizeof(others));
