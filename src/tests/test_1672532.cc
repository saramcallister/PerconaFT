...
#include "test.h"
//to verify the DB_LOCKING_READ works to lock the read rows for snapshot isolaton.
//we create a db, then init a read transaction with repeatable-read isolation and
//locking read flag, then we start another transaction to grab the write lock.
//DB_LOCKING_READ is defined here to just make the before and after tests work (before
...
static void test_read_write_range(DB_ENV *env, DB *db, uint32_t iso_flags, int expect_r) {
    int r;
    ...
    DBC *cursor_a = NULL;
    r = db->cursor(db, txn_a, &cursor_a, DB_LOCKING_READ); assert_zero(r);
    DBC *cursor_b = NULL;
...
static void test_read_write_point(DB_ENV *env, DB *db, uint32_t iso_flags, int expect_r) {
    int r;

    DB_TXN *txn1 = NULL;
    r = env->txn_begin(env, NULL, &txn1, iso_flags); assert_zero(r);

    DB_TXN *txn2 = NULL;
    r = env->txn_begin(env, NULL, &txn2, iso_flags); assert_zero(r);

    DBC *c1 = NULL;
    r = db->cursor(db, txn1, &c1, DB_LOCKING_READ); assert_zero(r);

    DBC *c2 = NULL;
    r = db->cursor(db, txn2, &c2, DB_RMW); assert_zero(r);
